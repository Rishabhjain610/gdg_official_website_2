...
import { IoHomeOutline, IoInformationCircleOutline, IoPeopleOutline, IoCalendarOutline, IoMailOutline } from 'react-icons/io5';

const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const pathname = usePathname();

  // Re-adding 'About' link for a complete navigation experience
  const navLinks = [
  ...
  return (
    <>
      {/* 
        Main Navigation Bar
        - Mobile/Tablet (default): Full-width, fixed top bar.
        - Desktop (md:): Becomes a floating, rounded pill in the center.
      */}
      <nav className={cn(
        "fixed z-50 w-[95%] top-4 rounded-2xl -translate-x-1/2 left-1/2 border-b border-white/10 bg-black/50 backdrop-blur-lg", // Base styles for mobile
        "md:top-4 md:left-1/2 md:w-[65%] md:-translate-x-1/2 lg:rounded-full md:border" // Overrides for desktop
      )}>
        <div className="flex h-16 items-center justify-between px-4 md:px-6">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2">
            <Image src="/Logo.png" alt="Website Logo" width={45} height={15} />
          ...
          <div className="flex lg:hidden">
            ...
            >
              ...
              <div className="relative h-5 w-5">
                {/* Each span is now centered vertically before being transformed, fixing the alignment. */}
                <span className={cn('absolute block h-0.5 w-5 transform bg-current transition duration-300 ease-in-out top-1/2 -translate-y-1/2', isOpen ? 'rotate-45' : '-translate-y-1.5')} />
                <span className={cn('absolute block h-0.5 w-5 transform bg-current transition duration-300 ease-in-out top-1/2 -translate-y-1/2', isOpen && 'opacity-0')} />
                <span className={cn('absolute block h-0.5 w-5 transform bg-current transition duration-300 ease-in-out top-1/2 -translate-y-1/2', isOpen ? '-rotate-45' : 'translate-y-1.5')} />
              </div>
            </button>
          </div>
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    pathname === link.href ? 'opacity-100 scale-100' : 'opacity-0 scale-75'
                  )}
                />
              </Link>
            ))}
